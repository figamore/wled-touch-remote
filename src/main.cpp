 ...
 private:
  uint8_t byteAt(uint32_t index) {
    ...
    if (loaded_y_ != int32_t(source_y)) {
      gfx.readRect(0, source_y, kScreenWidth, 1, row_rgb_);
      loaded_y_ = source_y;
    }

    const uint32_t x = row_byte / 3;
    const uint32_t component = row_byte % 3;
    const lgfx::rgb888_t& pixel = row_rgb_[x];
    return component == 0 ? pixel.r : component == 1 ? pixel.b : pixel.g;
  }
  ...
  int32_t loaded_y_ = -1;
  uint8_t header_[54];
  lgfx::rgb888_t row_rgb_[kScreenWidth];
};
